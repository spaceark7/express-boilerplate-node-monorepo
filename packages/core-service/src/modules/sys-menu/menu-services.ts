...
export class MenuService {
  ...
  private static getMenuInclude(depth: number = 3): any {
    if (depth <= 0) {
      return {};
    }
 ...
 */
  private static transformMenusForGroup(menus: any[], groupId: number, includeChild = false): any[] {
    return menus.map(menu => {
      // Find the specific group entry for this menu to get isActive status
      const groupEntry = menu.groups.find((g: any) => g.groupId === groupId);
  ...
  static async create(menuData: TSysMenuCreate, groupId?: number) {
    ...
    const newMenu = await prismaClient.$transaction(async (tx) => {
      ...
      if (activeGroups.length > 0) {
        await tx.menuGroup.createMany({
          data: activeGroups.map(group => ({
            menuId: newMenu.id,
            groupId: group.id,
            // If the specific group is provided and it matches this group, use the provided isActive
      ...
      const menuWithGroups = await tx.menu.findUnique({
        where: { id: newMenu.id },
        include: MenuService.getMenuInclude(0)
      });

      // 6. If a specific groupId is provided, transform the result to match the expected format
      return groupId !== undefined
        ? MenuService.transformMenusForGroup([menuWithGroups], groupId)[0]
        : menuWithGroups;
    });
  ...
  static async findById(menuId: number) {
 ...
 */
  static async findMenusByGroupId(
    groupId: number,
  ...
  static async updateOne(groupId: number, menuData: TSysMenuUpdate) {
